...
namespace AoC19
{
    public class Day05
    {
        ...
        The TEST diagnostic program will run on your existing Intcode computer
        ...
        */
        ...
        Parameter modes are single digits,
        one per parameter,
        ...
        The result of this operation, 33 * 3 = 99,
        is written according to the third parameter, 4 in position mode,
        which also works like it did before
        ...
        It will then perform a series of diagnostic tests
        confirming that various parts of the Intcode computer,
        like parameter modes, function correctly.
        For each test, it will run an output instruction
        ...
        If all outputs were zero except the diagnostic code,
        ...
        void Solves_puzzle()
        {
            ...
            Run(MyPuzzleInput, readInput: () => 1, writeOutput: x => output.Add(x));

            output.Last().Should().Be(12896948);
        }
        ...
        [Theory, AutoData]
        void OpCode_7_stores_zero_if_the_first_is_greater_than_second(int x, int y)
        {
            var first = Math.Max(x, y);
            var second = Math.Min(x, y);
        ...
        For example,
        here are several programs that take one input,
        compare it to the value 8,
        ...
        The program will then output 999
        if the input value is below 8,
        output 1000 if the input value is equal to 8,
        or output 1001 if the input value is greater than 8.

        This time, when the TEST diagnostic program runs its input instruction
        to get the ID of the system to test,
